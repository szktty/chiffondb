...
5. **Remove the probe by hand — never with a destructive git command.** Delete exactly
   the lines you added; do **not** run `git checkout -- <file>`, `git stash`, `git
   reset`, `git restore`, or `git clean` (they would wipe the implementer's uncommitted
   code). Leave formatting/lint clean. Never dirty production code during review.
...
8. If a failing test reveals a core bug, **do not fix it** — record it under Must fix
...
5. When fixed, send it back via `request` (do not commit yourself — committing happens
   in `close`).
